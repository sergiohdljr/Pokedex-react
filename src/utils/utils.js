...
// funções  para formatar texto sobre o pokemon

const formatPokemonAbout = (text) => {
  return text?.includes("POKéMON") ? text?.replace("POKéMON", "pokémon") : text;
};

export const formatarText = (text) => {
  if (text?.includes("\f")) {
    return formatPokemonAbout(text?.replace("\f", " "));
  } else {
    return formatPokemonAbout(text);
  }
};
